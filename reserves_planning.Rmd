...
The median abundance of the features is `r round(median_abundance, 2)`km^2. xxx...check if units show up twice when knitted
...
The feature with the smallest abundance is `r min_abun_feature`.
...
There are `r num_features_greater100km` that have a total abundance greater than 100 km/^2.
...
An average of `r avg_prop_protected`% of features are protected.
...
Based on current protected status, `r `num_features_lessthan_10pct` features fail to meet a target of 10% coverage by protected areas.
...
Based on current protected status, `r `num_features_lessthan_20pct` features fail to meet a target of 20% coverage by protected areas.
...
There does not appear to be a relationship between the total abundance of a feature and how well it is represented by protected areas.
...
prop_pu_selected_1
```
A total of `r num_pu_selected_1` planning units were selected in the prioritization. This represents `r prop_pu_selected_1`% of all planning units.
xxx
